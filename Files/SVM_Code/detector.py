...
class ObjectDetector(object):
    ...
    def predict(self,image):
	    ...
	    print str(x) +';' + str(y)+';'  +str(xb) +';' + str(yb)
	    self._pX[self._ind] = int( (x + xb)/2 )
	    self._pY[self._ind] = int( (y + yb)/2 )
    ...
    def detectsp(self,image,annotate=None):
        #image = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
        preds = self.predict(image)
        for (x,y,xb,yb) in preds:
            #image = cv2.cvtColor(image,cv2.COLOR_RGB2BGR)
            ...
            #draw path in image
	    for vx in range(0, self._ind):
		cv2.circle(image,(self._pX[vx],self._pY[vx]),(int)(3),(0,0,255),1)
